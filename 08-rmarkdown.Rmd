...
As you guess, changing text color in R Markdown is possible. There are many codes to do it. 
<li>If your output is an HTML file, you can wrap your text in the <span> tag and set color with CSS. For example: `<span style="color: red;">The quick brown fox jumps over the lazy dog</span>`.</li>
<li>If your output is a PDF file, you can use that chunk (don't show the code in your R Markdown, just run it) to write a custom R function for use in later parts of your R markdown document:</li>
...
You can then use the code in an inline R expression `` `r
colorize("The quick brown fox jumps over the lazy dog", "blue")` `` in the output, which will create <span style="color: blue;">The quick brown fox jumps over the lazy dog</span>.

Don’t be afraid! There is one more solution. Those codes seem to work very well in both output formats, PDF and HTML:
`Roses are $\color{red}{\text{red}}$, violets are $\color{blue}{\text{blue}}$, some poems rhyme, this one doesn’t.`
<p>Output:
Roses are $\color{red}{\text{red}}$, violets are $\color{blue}{\text{blue}}$, some poems rhyme, this one doesn’t. Notice: These codes may change the font as well as the colour of the text.</p>

As mentioned before, you can colour your text with HEX codes.
`Roses are $\color{#8B0B0B}{\text{red}}$, violets are $\color{#000F64 }{\text{blue}}$, some poems rhyme, this one doesn’t.`
<p>Output:
Roses are $\color{#8B0B0B}{\text{red}}$, violets are $\color{#000F64 }{\text{blue}}$, some poems rhyme, this one doesn’t.</p>
